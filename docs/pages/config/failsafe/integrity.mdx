...
**Large range handling**: eRPC can auto-split large ranges based on [`getLogsAutoSplittingRangeThreshold`](/config/projects/upstreams#eth_getlogs-max-range-automatic-splitting) or when an upstream returns "range too large" errors. A parallel [`traceFilterAutoSplittingRangeThreshold`](/config/projects/networks) controls the same behavior for trace requests.
