...
server:
  ...
  #                        alive for this duration so load-balancers can gracefully
  #                        close any still-open TCP connections.
...
In verbose mode, the healthcheck returns a detailed JSON response with information about the status of each project and upstream, including the dynamically estimated block time per network:
...
The `blockTimeMs` field shows the EMA-estimated block time in milliseconds for each network, computed from on-chain block timestamps. This value is `null` (omitted) during startup until enough block observations have been collected.
