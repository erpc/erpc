...
Cordon state lives on the health tracker as a set of owner → `{reason, cordonedAt}`
entries per `(upstream, method, finality=All)` cell, where `method` defaults to `"*"` for
whole-upstream cordons. The cell is cordoned while any owner holds an entry. Two owners
exist: `admin` (the `erpc_cordonUpstream` RPC) and `auto` (in-process detectors: consensus
sit-out, state-poller health and chain-identity checks). Owners never clobber each other —
a consensus penalty ending cannot lift an operator cordon. `erpc_uncordonUpstream` is the
operator override: it lifts every owner on the cell, which is also how an automatic cordon
with no self-heal path (a chain-identity mismatch) is cleared without a restart. It is not a rolling-window metric: `Rotate()` (the window-tick
...
The last owner leaving records the duration (`erpc_upstream_cordon_duration_seconds`) and
emits the `action="uncordon"` event counter. Repeated cordon calls by the same owner
...
**Cross-replica persistence.** When `database.sharedState` uses a remote connector
(`redis`, `postgresql`, `dynamodb`), operator cordons are persisted as one record per
project (`<clusterKey>/cordons/<projectId>` → `upstreamId → method → {reason, cordonedAtMs}`)
and every replica polls that record every 15 s (`upstream.cordonSyncInterval`), applying
`admin` entries it lacks and lifting `admin` entries no longer present. `erpc_cordonUpstream`
persists first and applies locally only after the write succeeds: if the store is
unreachable the RPC returns an error and nothing changes, so a pod never holds a cordon its
peers will not. A restarted or newly scaled pod converges on its first tick. With the
`memory` driver (the default when `sharedState` is omitted) there is no persistence and
cordons are in-process only.
...
- **Do not use cordon for permanent exclusions.** Even when persisted through shared state a
  cordon is operational state, not config. For a vendor you want permanently removed, update
  the config.
- **Run a remote shared-state connector when you run more than one replica.** Without it
  each `erpc_cordonUpstream` call only reaches the pod that served it.
...
3. **Reason change moves the gauge series.** `erpc_upstream_cordoned` is labeled by
   `reason`; re-cordoning with a new reason sets the old reason's series to `0` and the new
   one to `1`, so alerts keyed on label value follow the edit.
4. **Cordon survives `Rotate()`; restart survival depends on shared state.** With a remote
   `database.sharedState` connector, operator cordons are restored within one sync tick
   (15 s) of a pod starting; with the `memory` driver they are lost on restart. Automatic
   cordons are never persisted: they are re-detected by the pod that observes the fault.
   Between a pod starting and its first sync tick, requests can reach an upstream other
   replicas have cordoned.
...
|---|---|---|---|
| `erpc_upstream_cordoned` | gauge | project, vendor, network, upstream, category (=method), reason | 1 while any owner holds that reason on the cell, 0 otherwise; per pod |
| `erpc_upstream_cordon_event_total` | counter | project, network, upstream, action | Effective edge transitions only: first owner in (`action="cordon"`), last owner out (`action="uncordon"`); repeated cordons and second owners do not increment. Each replica counts its own transitions, so a persisted cordon shows one event per pod |
