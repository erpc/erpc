...
- [`infura`](#infura-json-rpc) A special upstream type that accepts Infura.io API-KEY and automatically adds all their EVM chains.
    ...
    upstreams:
      - id: blastapi-chain-42161
        ...
        evm:
          ...
          chainId: 42161
          # statePollerInterval is required and can be used to change the interval for checking the node's liveness. Default is 30s.
          statePollerInterval: 5s
          # nodeType is optional and you can manually set it to "full" or "archive"
          nodeType: full
          # maxAvailableRecentBlocks is optional and can be used to limit the maximum number of recent blocks to be served by this upstream. Default is 128 for full nodes.
...
<br />
- [ ] Introduce a programmable (Typescript) routing selection policy to include/exclude upstreams based on some business logic (e.g. only use expensive nodes if cheap nodes are fully down for a long time).
