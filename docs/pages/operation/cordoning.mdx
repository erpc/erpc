...
When a `sharedState` connector is configured, admin cordon state is persisted to Redis and
...
11. **Cross-replica propagation has a sub-second delay.** `erpc_cordonUpstream` writes to
    Redis and publishes a counter bump via pub/sub. Other replicas receive the notification
    nearly immediately and call `reconcileCordonState`, but there is a short window (typically
    &lt;100 ms, bounded by Redis round-trip) during which a replica may still route to an
    upstream that was just cordoned on another replica. Build runbooks with this in mind: wait
    at least one eval interval (15 s by default) after issuing an admin cordon before
    assuming all replicas have stopped routing.
