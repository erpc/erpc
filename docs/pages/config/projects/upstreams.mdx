...
Per-upstream score weighting lives next to the upstream itself, in `routing.scoreMultipliers`. Each entry selects on `(network, method, finality)` and overrides the score-weight preset's dimension(s) for that upstream when the selector matches the current request.
  ...
  - id: hot
    ...
    routing:
      scoreMultipliers:
        - network: "*"
          method: "*"
          errorRate: 8            # overrides PREFER_FASTEST.errorRate
          respLatency: 12         # overrides PREFER_FASTEST.respLatency
          # missing dimensions inherit from the preset
  - id: cold
    endpoint: drpc://...
    routing:
      scoreMultipliers:
        - network: "*"
          method: "*"
          respLatency: 4          # mild latency preference; everything else inherits
```

`PREFER_FASTEST` (the default preset) is `{ errorRate: 4, respLatency: 8, throttledRate: 3, blockHeadLag: 2, finalizationLag: 1, misbehaviors: 6 }`. Latency dominates because the `excludeIf` chain already filters degraded upstreams before scoring runs; among survivors, response speed is the strongest "which should I prefer?" signal. Per-upstream entries override on a per-dimension basis; unspecified dimensions inherit.

When you need branching that the declarative form can't express (per-method, vendor, or `ctx.finality` rules), pass a function to `sortByScore` directly:

```js
(upstreams, ctx) =>
  upstreams.sortByScore((u) => {
    if (u.id === 'noisy' && ctx.method === 'eth_getLogs') return { respLatency: 16, errorRate: 4 };
    return PREFER_FASTEST;
  })
```

The function form takes full control — it does NOT auto-merge with `routing.scoreMultipliers`. Pick one or the other per upstream.
