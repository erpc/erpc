...
database:
  evmJsonRpcCache:
    ...
    policies:
      # New matcher-based format (recommended)
      - matchers:
          - network: string # Optional
            method: string # Optional  
            params: []any # Optional
            finality: []finality # Optional - array of [finalized | unfinalized | realtime | unknown]
            empty: ignore | allow | only # Optional
            action: include | exclude # Optional (default: include)
...
database:
  evmJsonRpcCache:
    ...
    policies:
      ...
      # New matcher-based examples:
      # Example: Cache everything except eth_getLogs
      - matchers:
          - method: "*"
            action: include
          - method: "eth_getLogs"
      ...
      # Example: Cache only specific methods for specific networks with different finality states
      - matchers:
          - network: "1"
          ...
          - method: "eth_getTransactionReceipt"
            finality: [finalized, unfinalized]
            action: include
        connector: redis-cache
        ttl: 5m
        
      # Example: Advanced caching with empty behavior and size limits
      - matchers:
          - method: "eth_call"
            empty: ignore  # Don't cache empty results
            action: include
          - method: "eth_getCode"
            empty: only    # Only cache empty results (useful for EOA checks)
            action: include
        connector: memory-cache
        minItemSize: 100B    # Don't cache tiny responses
        maxItemSize: 1MB     # Don't cache huge responses
        ttl: 1h
