    ...
    upstreams:
      ...
      - endpoint: https://arbitrum-one.blastapi.io/xxxxxxx-xxxxxx-xxxxxxx
        ...
        evm:
          ...
          # (OPTIONAL) getLogsSplitOnError is used to split the getLogs request if upstream complains about the range being too large,
          # or if the request timeouts due to too many addresses/topics.
...
#### `eth_getLogs` proactive auto-splitting
...
#### `eth_getLogs` auto-split on error
...
To enable this behavior, set the `getLogsSplitOnError` in the upstream config:
...
#### `eth_getLogs` max allowed block range

To enforce a maximum block range for `eth_getLogs` requests towards your eRPC instance, and return an error to the user use `getLogsMaxAllowedRange` config:
...
To enforce a maximum number of addresses allowed for `eth_getLogs` requests towards your eRPC instance, and return an error to the user use `getLogsMaxAllowedAddresses` config:
...
To enforce a maximum number of topics allowed for `eth_getLogs` requests towards your eRPC instance, and return an error to the user use `getLogsMaxAllowedTopics` config:
