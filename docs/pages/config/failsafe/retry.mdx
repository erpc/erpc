...
- **Empty-result handling.** The field is `emptyResultAccept`. For network-wide control, set `directiveDefaults.retryEmpty: false` (or per-request `?retryEmpty=false`) instead.
