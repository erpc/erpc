  ...
  - id: main
    ...
    networks:
      - architecture: evm
        ...
        # (OPTIONAL) Per-request failover between upstream groups.
        # When onDefaultsExhausted is true, the request loop tries upstreams with
        # group "default" (or unset) first and only advances to group "fallback"
        # if every default upstream returned a retryable error within the same
        # request. Deterministic client errors (reverts, invalid params, etc.)
        # short-circuit without advancing.
        #
        # This is separate from selectionPolicy above — failover operates per
        # request, while selectionPolicy evaluates group membership on an
        # interval across requests. When failover is enabled, the default
        # auto-applied selectionPolicy that filters out fallbacks is suppressed
        # so fallbacks remain visible to the request loop.
        #
        # DEFAULT: false (legacy behaviour; selectionPolicy handles group gating)
        failover:
      ...
      networks: [
        {
          ...
          */
          ...
          /**
          * (OPTIONAL) Per-request failover between upstream groups.
          * When onDefaultsExhausted is true, the request loop tries upstreams
          * with group "default" (or unset) first and only advances to group
          * "fallback" if every default upstream returned a retryable error
          * within the same request. Deterministic client errors short-circuit
          * without advancing.
          *
          * Independent of selectionPolicy — failover is per-request;
          * selectionPolicy evaluates group membership on an interval. When
          * failover is enabled, the default auto-applied selectionPolicy that
          * filters out fallbacks is suppressed.
          *
          * DEFAULT: false (legacy behaviour; selectionPolicy handles group gating)
          */
          failover: {
...
projects:
  - id: main
    networkDefaults:
      ...
      # (OPTIONAL) Per-request failover between upstream groups, applied to
      # every network unless overridden at the network level. See the per-network
      # `failover` docs above for full semantics.
      # Defaults to disabled (legacy behaviour).
      failover:
        onDefaultsExhausted: true
  ...
  projects: [
    {
      ...
      networkDefaults: {
        ...
        // (OPTIONAL) Per-request failover between upstream groups, applied to
        // every network unless overridden at the network level. See the
        // per-network `failover` docs above for full semantics.
        // Defaults to disabled (legacy behaviour).
        failover: {
          onDefaultsExhausted: true,
        },
