...
| `metrics.customizations` | []object | nil | Ordered list of `{subject, action, labels, buckets}` entries controlling which metric families are exposed, which labels they carry, and their histogram buckets. `subject` is a metric name without the `erpc_` prefix, optionally ending in `*`. |
