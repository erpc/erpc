...
| Selection policy | The eval's per-tick `Decision.Output.Annotations[id]` includes the cordon reason on the excluded row. Surfaces in the simulator's policy-history modal as a pill. |
