...
2. **Budget/coalescing**: token-bucket `budget.maxPerSecond`, `budget.maxConcurrent`, single-flight per block, `enforceFrom` default (finalized); graceful degradation to `corroborated` on budget exhaustion.
