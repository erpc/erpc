...
This directive is "false" by default, which means cache will be used. To skip a cache read for certain requests, you can set the `X-ERPC-Skip-Cache-Read` header to `true`. Useful when you need to force-refresh some data or override an already cached response.
