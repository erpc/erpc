...
* `ROUTING_POLICY_MAX_BLOCK_HEAD_LAG` (Default: `10`): Maximum allowed block head lag, expressed as a number of blocks behind the network's highest known block (not seconds). Tolerances differ per chain — e.g. `10` is ≈120s on Ethereum but ≈2.5s on Arbitrum.
...
<Callout type='info'>
  If you want fallbacks to kick in immediately when defaults fail (rather than waiting for the next interval), use [`failover.onDefaultsExhausted`](/config/projects/networks#config) on the network config instead.
</Callout>
...
export type UpstreamMetrics = {
    ...
    // Note: this is a block-number delta, not seconds. Tolerances differ per chain
