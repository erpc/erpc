...
func (c *counterInt64) TryUpdateIfStale(ctx context.Context, staleness time.Duration, executeNewValueFn func(ctx context.Context) (int64, error)) (int64, error) {
	...
	lockCancel()
	localFallbackStartedAt := time.Time{}
	observeLocalFallback := func() {
		if localFallbackStartedAt.IsZero() {
			return
		}
	...
	if unlock == nil {
		if isContention {
			...
			// If no fresh value arrives within updateMaxWait, the current (potentially stale)
			// value is returned without advancing the staleness timestamp, allowing
...
func (c *counterInt64) tryAcquirePollLock(ctx context.Context) (func(), bool) {
	...
	observeWait := func(outcome string) {
		telemetry.ObserverHandle(
			telemetry.MetricSharedStatePollLockWaitDuration,
			outcome,
		).Observe(time.Since(waitStartedAt).Seconds())
	}
	...
	return func() {
		...
		defer func() {
			telemetry.ObserverHandle(
				...
				holdOutcome,
			).Observe(time.Since(holdStartedAt).Seconds())
		}()
		// Unlock timeout uses base lockTtl (not pollTTL).
		unlockCtx, cancel := context.WithTimeout(c.registry.appCtx, c.registry.lockTtl)
		defer cancel()
		if err := lock.Unlock(unlockCtx); err != nil {
			if errors.Is(err, ErrLockExpired) {
