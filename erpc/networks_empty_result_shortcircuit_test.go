...
func TestEmptyResultAcceptShortCircuit(t *testing.T) {
	t.Run("EthGetLogs_EmptyArray_TriesAllUpstreamsBeforeReturningEmpty", func(t *testing.T) {
		util.ResetGock()
		...
		assert.Equal(t, int32(1), rpc2Calls.Load(), "rpc2 should also be called before accepting empty eth_getLogs")
		assert.Less(t, elapsed, 500*time.Millisecond, "should return immediately without trying rpc2")
	})

	t.Run("EthGetLogs_FirstEmpty_SecondHasLogs_ReturnsSecondResponse", func(t *testing.T) {
		util.ResetGock()
		defer util.ResetGock()
		util.SetupMocksForEvmStatePoller()
		...
		resp, err := network.Forward(ctx, req)
		require.NoError(t, err, "eth_getLogs should continue to rpc2 when rpc1 returns empty")
		require.NotNil(t, resp)

		jrr, jrrErr := resp.JsonRpcResponse()
		require.NoError(t, jrrErr)
		assert.False(t, jrr.IsResultEmptyish(), "result should contain logs from rpc2")
		assert.Contains(t, jrr.GetResultString(), "logIndex")
		assert.Equal(t, int32(1), rpc1Calls.Load(), "rpc1 should be called once")
		assert.Equal(t, int32(1), rpc2Calls.Load(), "rpc2 should be called once after rpc1 returns empty")
	})
	...
	t.Run("DefaultAcceptList_EthGetLogs_TriesAllUpstreamsWithoutExplicitConfig", func(t *testing.T) {
		util.ResetGock()
		defer util.ResetGock()
		util.SetupMocksForEvmStatePoller()
