...
For zero-downtime deployments behind any L7 load balancer, pair a readiness probe that fails quickly (≤ 10s) with generous shutdown windows. This gives the load balancer enough time to stop routing new requests before the process exits, and lets in-flight requests drain:

```yaml
server:
  waitBeforeShutdown: 30s # pod is in draining mode (readiness fails, but serving continues)
  waitAfterShutdown:  30s # process stays alive after the listener is closed
```

Shorter values risk the load balancer reusing a connection after the listener is gone, or routing new requests to a pod that has already exited. Use these numbers as a safe starting point and adjust to match your own probe intervals.
