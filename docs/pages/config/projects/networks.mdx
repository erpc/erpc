...
selectionPolicy:
  ...
  evalPerMethod: false        # if true, run evalFunc per (network, method) pair
  evalFunc: |
    (upstreams, ctx) =>
      upstreams
...
`evalFunc` returns an **ordered** `Upstream[]`: position 0 is the primary, the rest are retry order, anything missing is excluded. Use the chainable stdlib (`sortByScore`, `preferTag`, `stickyPrimary`, `probeExcluded`, `removeCordoned`, `keepHealthy`, `whenEmpty`, …) — see [Selection policies](/config/projects/selection-policies) for the full DSL.

Default policy: omitting `selectionPolicy` (or `evalFunc`) applies a production-hardened chain-agnostic default — `keepHealthy` + tier-based fallback via `preferTag('!tier:fallback')` + `sortByScore(BALANCED)` + sticky primary + probe re-admit. Investigations are exposed via OTLP tracing and the `policy_selection_*` Prometheus metric family.

**Legacy.** Older configs with `evalFunction` / `decisionHistory` / `resampleExcluded` / `group: "fallback"` / `ROUTING_POLICY_*` env vars are still accepted at load time and rewritten into the new shape (a deprecation warning is logged). See the [migration guide](/migration/selection-policy).
