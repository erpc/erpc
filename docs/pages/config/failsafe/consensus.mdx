...
|---|---|---|
| `maxWaitOnResult` | [DurationSpec](/config/failsafe#durationspec---reusable-duration-with-quantile-and-bounds) | At least one **non-empty** response is in the bag. |
| `maxWaitOnEmpty` | [DurationSpec](/config/failsafe#durationspec---reusable-duration-with-quantile-and-bounds) | The first response (of any kind — empty, error, or non-empty) is in the bag. |
