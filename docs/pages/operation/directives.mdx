...
<Callout type="info">
  Cache max-age primarily applies to data with **realtime finality** (e.g., `latest` block queries). For finalized data, default cache max-age settings are ignored and responses are served from cache because the data is immutable. However, if the client explicitly sets a cache max-age (for example via `X-ERPC-Cache-Max-Age` or `cache-max-age`), that explicit directive can still force a refresh when the cached entry is older than the specified max age.
</Callout>
