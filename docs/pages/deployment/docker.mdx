...
### Validate config with validator image

Run config validation with the dedicated validator image:

```bash
docker run --rm \
  -v $(pwd)/erpc.yaml:/erpc.yaml \
  ghcr.io/erpc/erpc-validator:latest \
  /usr/local/bin/erpc validate --config /erpc.yaml --format md
```
