...
type SolanaNetworkConfig struct {
	...
	// NOTE: fallbackFinalitySlotDistance and statePollerDebounce were declared
	// here but never wired (the poller hardcodes a 400ms debounce, see
	// architecture/solana/state_poller.go defaultSlotDebounce). They were
	// removed rather than shipped as silent no-op knobs; re-add them together
	// with the plumbing into NewSolanaStatePoller when they're actually honored.
}
