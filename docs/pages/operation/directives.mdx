...
| 8 | `X-ERPC-Enforce-GetLogs-Range` | `enforce-getlogs-range` | bool | `enforceGetLogsBlockRange` | **`true`** (SetDefaults) | Pre-forward: rejects `eth_getLogs`/`trace_filter` if the range is unavailable on the upstream. Honors `directiveDefaults.enforceGetLogsBlockRange` (deprecated `evm.integrity` is fallback only). | [`architecture/evm/eth_getLogs.go`](https://github.com/erpc/erpc/blob/main/architecture/evm/eth_getLogs.go) |
...
6. **`directiveDefaults.enforceGetLogsBlockRange: false` disables the getLogs range hook.** The hook prefers that flag (and a request directive of `true`) over deprecated `evm.integrity.enforceGetLogsBlockRange`. `Integrity.SetDefaults` still fills Integrity to `true` when unset; SetDefaults then copies DirectiveDefaults onto Integrity so the two cannot diverge. Source: [`architecture/evm/eth_getLogs.go`](https://github.com/erpc/erpc/blob/main/architecture/evm/eth_getLogs.go), [`common/defaults.go`](https://github.com/erpc/erpc/blob/main/common/defaults.go).
