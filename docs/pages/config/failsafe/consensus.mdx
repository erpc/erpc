...
<Callout type="info">
  Response comparison uses canonical JSON-RPC hashing. When `preferNonEmpty` is enabled (default), **non-empty responses are preferred over empty ones**, regardless of count (e.g., 1 non-empty beats 3 empty responses).
</Callout>

## Behavior options

### `preferNonEmpty`
When enabled (default: `true`), non-empty responses are preferred over empty ones, regardless of count. For example, 1 non-empty response beats 3 empty responses. When disabled, all response types are treated equally and only count/size matter.

### `preferLargerResponses` 
When enabled (default: `true`), larger responses (more data) are preferred among those meeting the `agreementThreshold`. If multiple responses meet the threshold, the one with more content wins. This helps ensure you get the most complete data available.

### `disputeBehavior`
When upstreams disagree (no consensus reached), sorted from most loose to strictest:

- **`acceptAnyValidResult`**: Returns any valid response. Respects `preferNonEmpty` setting.
- **`acceptMostCommonValidResult`**: Returns most common response if it meets `agreementThreshold`. Respects preference settings. Errors if no clear winner.
...
When fewer than `agreementThreshold` responses are available, sorted from most loose to strictest:

- **`acceptAnyValidResult`**: Uses any available upstream. Respects preference settings. Maximum availability.
- **`acceptMostCommonValidResult`**: Uses available upstreams, returns most common if meets threshold. Respects preference settings and clear winner rules.
