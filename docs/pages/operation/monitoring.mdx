...
| erpc_network_latest_block_timestamp_distance_seconds | Gauge   | Distance in seconds between the latest block timestamp and current time for a network. Useful for monitoring chain synchronization and block production latency.                              |
