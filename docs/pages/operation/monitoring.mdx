...
| erpc_upstream_evm_get_logs_range_exceeded_hard_limit_total | Counter   | Total number of times eth_getLogs request exceeded the block range hard limit will not be split further (based on upstream config for "upstream.evm.GetLogsMaxAllowedRange"). |
