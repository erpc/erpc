...
| erpc_selection_score                               | Gauge     | Per-upstream score from `sortByScore`. Lower = better. Missing for upstreams that bypassed scoring (probeExcluded/forceInclude additions). Labels: project, network, method, upstream.         |
...
| `erpc_selection_score{upstream}` | Gauge | Penalty from `sortByScore`. **Lower = better.** Missing for upstreams added after the scoring step. |
