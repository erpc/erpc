...
* Manual splitting based on [`upstream.evm.getLogsMaxBlockRange` config](/config/projects/upstreams#eth_getlogs-max-range-automatic-splitting) per-upstream when you what is the max block range they support.
...
- `erpc_upstream_evm_get_logs_range_exceeded_total` - Total number of times eth_getLogs request exceeded the maximum allowed block range and needed splitting (based on upstream config for "upstream.evm.getLogsMaxBlockRange").
