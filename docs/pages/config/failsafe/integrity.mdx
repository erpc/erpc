...
| `validateLogsBloomEmptiness` | `directiveDefaults` | `true` | Reject inconsistent bloom/log emptiness when logs are available to validate |
...
For block-level bloom validation on methods like `eth_getBlockByNumber`, `GroundTruthLogs` must represent the **complete** block log set. Filtered log queries are not sufficient for bloom consistency checks.

Set via **config** (applies to all requests), **HTTP headers**, or **query parameters**:

| Directive | Header | Query | Description |
|-----------|--------|-------|-------------|
| `validateLogsBloomEmptiness` | `X-ERPC-Validate-Logs-Bloom-Emptiness` | `validate-logs-bloom-emptiness` | Bloom/logs consistency using `GroundTruthLogs`: logs exist ↔ bloom non-zero |
    ...
    networks:
      - architecture: evm
        evm:
          chainId: 1
        directiveDefaults:
          # Bloom/log emptiness validation is enabled by default.
          # Full bloom recomputation remains opt-in for high-integrity use-cases:
