...
|---|---|
| `finalized` | `min(finalizedTip, indexedTip)`; when the indexed tip is unknown, `finalizedTip − 32`. **Clamped from both directions** — stale values are raised, values above the floor are capped down. `finalizedTip` here is the **majority** tip, because this value is advertised to clients and a lone rogue upstream must not inflate it. |
| `confirmed` | **Passed through uncorrected.** |
| `processed` | The processed tip (`LatestSlot()`). Stale values are raised only; a value above the tip is left alone. |

"Effective" includes the level the node applies when the request pins none: Solana's documented default is `finalized`, so an **unset** commitment takes the `finalized` row, not the `processed` one. Flooring an unpinned `getSlot` at the processed tip advertised a slot ~60 above the finalized root — exactly the band the `getBlock` guard below rejects, so eRPC handed clients a head it then refused to serve.
...
**`getBlock` availability guard.** An upstream returns `-32004` both for a slot above its `maxShredInsertSlot` and for a slot above its own finalized root, so hitting all upstreams only to collect N identical `-32004`s wastes quota and delays the retry. When the requested slot is above what the pool can serve, `getBlock`/`getConfirmedBlock` are short-circuited with `ErrEndpointMissingData` before any upstream call — the same error class that maps to the `missing_data` retry reason, so the indexing-lag retry fires immediately.

The bound is **commitment-dependent**, on the same effective commitment as the `getSlot` correction:

| Effective commitment | Bound |
|---|---|
| `finalized` (including unpinned) | `min(maxFinalizedRoot, indexedFrontier)` |
| `confirmed`, `processed` | `indexedFrontier` alone — slots between the finalized and indexed frontiers *are* held at those levels |

Both inputs are **MAX across upstreams**, never the majority tip: this is a rejection gate, so its only job is to avoid short-circuiting a slot that *some* upstream can serve, and the most-advanced upstream sets the bound. With roots skewed `1000/1000/1100`, the majority tip is `1000` and bounding by it would null out ~100 slots the third upstream is already holding. The advertising side above uses the majority tip, so every slot eRPC advertises is at or below the bound it serves.
...
The guard **fails open** whenever the tip it needs is unknown — an indexed frontier of `0` forwards every slot, and a finalized root of `0` falls back to the indexed bound rather than collapsing to slot `0`. Absence of tracking is not evidence of unavailability, so a cold poller or a vendor without `getMaxShredInsertSlot` must not cost requests every upstream could serve.
...
- [`architecture/svm/hooks.go`](https://github.com/erpc/erpc/blob/main/architecture/svm/hooks.go) — `networkPostForward_getSlot` (per-commitment correction), `networkPreForward_getBlock` (availability guard), `effectiveCommitment`, `tipStalenessMargin`, `upstreamPostForward_trackContextSlot`.
