...
func setupTestProjectWithIntentionalBlockLag(t *testing.T, ctx context.Context, blockLag int64, latestBlock int64) *PreparedProject {
	...
	vr := thirdparty.NewVendorsRegistry()
	_, err = thirdparty.NewProvidersRegistry(&log.Logger, vr, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	prjReg, err := NewProjectsRegistry(
		ctx,
		&log.Logger,
		[]*common.ProjectConfig{projectConfig},
		ssr,
		nil,
		rlr,
	...
	project, err := prjReg.GetProject("test-project")
	if err != nil {
		t.Fatal(err)
	}

	// Set the latest block on the upstream
	network, err := project.GetNetwork("evm:123")
	if err != nil {
		t.Fatal(err)
	}
	upsList := network.upstreamsRegistry.GetNetworkUpstreams(context.TODO(), util.EvmNetworkId(123))
	upsList[0].EvmStatePoller().SuggestLatestBlock(latestBlock)

	return project
}

func TestNetwork_IntentionalBlockLag(t *testing.T) {
	t.Run("IntentionalBlockLag_GetBlockByNumber_Latest", func(t *testing.T) {
		...
		// Create a project with intentional block lag
		project := setupTestProjectWithIntentionalBlockLag(t, ctx, 100, 200)
