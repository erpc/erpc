...
projects:
  - id: main
    ...
    upstreams:
      ...
      - endpoint: alchemy://your-api-key
      - endpoint: tenderly://your-api-key
      - endpoint: https://mainnet.gateway.tenderly.co/your-api-key
        ...
        evm:
          ...
          # (OPTIONAL) getLogsAutoSplittingRangeThreshold limits the maximum block range for eth_getLogs requests.
          ...
          # (OPTIONAL) getLogsMaxAllowedRange limits the maximum block range for eth_getLogs requests.
          # This is used to reject getLogs request if the range is beyond the limit and returns an error to the caller of eRPC.
          getLogsMaxAllowedRange: 10000
          # (OPTIONAL) getLogsMaxAllowedAddresses limits the maximum number of addresses for eth_getLogs requests.
          # This is used to reject getLogs request if the number of addresses is beyond the limit and returns an error to the caller of eRPC.
          getLogsMaxAllowedAddresses: 10000
          # (OPTIONAL) getLogsMaxAllowedTopics limits the maximum number of topics for eth_getLogs requests.
          # This is used to reject getLogs request if the number of topics is beyond the limit and returns an error to the caller of eRPC.
...
#### `eth_getLogs` max range automatic splitting
...
To enable this feature, set the `getLogsAutoSplittingRangeThreshold` in the upstream config:
...
Additionally eRPC has integrity checks for eth_getLogs you can read more about in the [Integrity](/config/failsafe/integrity#eth_getLogs-behavior) section.
...
#### `eth_getLogs` max allowed range

Certain providers have a limit on the maximum block range for `eth_getLogs` requests. If the range is beyond the limit, eRPC will reject the request and will not be split further.

To set this limit, use the `getLogsMaxAllowedRange` config:
...
#### `eth_getLogs` max allowed addresses

Certain providers have a limit on the maximum number of addresses for `eth_getLogs` requests. If the number of addresses is beyond the limit, eRPC will reject the request and will not be split further.

To set this limit, use the `getLogsMaxAllowedAddresses` config:
...
#### `eth_getLogs` max allowed topics

Certain providers have a limit on the maximum number of topics for `eth_getLogs` requests. If the number of topics is beyond the limit, eRPC will reject the request and will not be split further.

To set this limit, use the `getLogsMaxAllowedTopics` config:
