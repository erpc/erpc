        ...
        failsafe:
          - matchMethod: "*"
            ...
            hedge:
              delay: 500ms
              maxCount: 1
            # Trip degraded upstreams out of rotation via the Selection Policy
            # (excludeIf chain), not failsafe — see
            # /config/projects/selection-policies. Operator-driven manual
            # exclusion lives in /operation/cordoning.
      - architecture: evm
        evm:
          chainId: 42161
        failsafe:
        ...
        failsafe:
          - matchMethod: "*"
            ...
            retry:
              ...
              backoffFactor: 1.2
              jitter: 0ms
            # NOTE: "trip a bad upstream out of rotation" is the
            # Selection Policy's job — the default policy already drops
            # upstreams with errorRate > 50%, throttling > 30%, p95 > 30s,
            # or block-head lag > 16 blocks. Just leave the defaults on
            # and use /operation/cordoning for manual operator overrides.
  ...
  projects: [{
    ...
    networks: [{
      ...
      failsafe: [{
        ...
        // Tripping a bad upstream out of rotation is the Selection
        // Policy's job — see /config/projects/selection-policies. The
        // default policy already covers errorRate / throttle / p95 / lag.
      }],
    }],
    upstreams: [
      {
...
- [Failsafe](/config/failsafe) — timeout, retry, hedge, consensus, integrity in depth
- [Selection policy](/config/projects/selection-policies) — how degraded upstreams get tripped out of rotation (replaces the old `circuitBreaker`)
- [Cordoning](/operation/cordoning) — manual operator-driven exclusion via admin RPC
...
**To trip a degraded upstream out of rotation** (the old `circuitBreaker`), use the [Selection Policy](/config/projects/selection-policies)'s `excludeIf` chain — the default policy already drops upstreams whose errorRate, throttling, p95 latency, or block-head lag cross sensible thresholds. For manual operator-driven exclusion (planned maintenance, incident response), use the [admin cordon RPC](/operation/cordoning).
