...
func (s *HttpServer) createRequestHandler() http.Handler {
	handleRequest := func(httpCtx context.Context, r *http.Request, w http.ResponseWriter, writeFatalError func(ctx context.Context, statusCode int, body error)) {
		...
		if isBatch {
			...
			if err != nil {
				// Keep transport 200 for JSON-RPC responses, even on error bodies
				writeFatalError(httpCtx, http.StatusOK, err)
				return
			} else {
				common.EnrichHTTPServerSpan(httpCtx, http.StatusOK, nil)
			}
...
func handleErrorResponse(
	...
	statusCode := http.StatusOK
	if common.HasErrorCode(err, common.ErrCodeInvalidUrlPath) {
		statusCode = http.StatusBadRequest
	} else if common.HasErrorCode(err, common.ErrCodeAuthUnauthorized) || common.HasErrorCode(err, common.ErrCodeEndpointUnauthorized) {
		statusCode = http.StatusUnauthorized
	} else if common.HasErrorCode(err, common.ErrCodeProjectNotFound) || common.HasErrorCode(err, common.ErrorCode("ErrNetworkNotFound")) {
