...
Configure [retry](/config/failsafe#retry-policy) at both levels:

- **Network-level** — retries across upstreams on hard failures. Set `maxCount` equal to the number of configured upstreams.
- **Upstream-level** — covers transient issues with a single upstream. Set `maxCount` between 2 and 5.

Set a [timeout](/config/failsafe#timeout-policy) that matches your request profile. For standard EVM calls a `3s` default is safe; for heavy trace or `getLogs` calls allow 10 s or more.

Enable the [hedge policy](/config/failsafe#hedge-policy) when you want the fastest possible response. A `500ms` delay fires a second upstream in parallel if the first hasn't replied — eRPC responds with whichever finishes first. Note: hedging increases upstream request volume.
