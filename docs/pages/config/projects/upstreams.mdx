    ...
    upstreams:
      ...
      - endpoint: https://arbitrum-one.blastapi.io/xxxxxxx-xxxxxx-xxxxxxx
        ...
        jsonRpc:
          ...
          # (OPTIONAL) Route all outbound requests to this upstream
          # through the provided HTTP proxy
          proxyUrl: http://proxy.myorg.local:3128
...
For use cases where you need to route traffic through a specific HTTP proxy, you can set a `proxyUrl` field under `jsonRpc`. When defined, eRPC will configure the HTTP client for that upstream to use the proxy.
  ...
  upstreams: [
    {
      ...
      endpoint: "https://public-rpc.io",
      jsonRpc: {
        // (OPTIONAL) Route all outbound requests through this HTTP proxy
        proxyUrl: "http://proxy.myorg.local:3128",
      },
    },
  ],
...
When proxyUrl is set, eRPC configures an internal HTTP transport so that all outbound calls for that upstream go via the specified proxy server (e.g. http://proxy.myorg.local:3128). If proxyUrl is absent or empty, no proxy is used. This is helpful for scenarios requiring IP-based filtering, private traffic routing, or custom load balancing.
