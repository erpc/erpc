  ...
  - id: main
    ...
    upstreams:
      ...
      - endpoint: https://arbitrum-one.blastapi.io/xxxxxxx-xxxxxx-xxxxxxx
        ...
        # Each upstream carries arbitrary label tags consumed by the
        # selectionPolicy stdlib (byTag / preferTag / spreadAcrossTags).
        # Convention: `<dimension>:<value>`. Use `tier:fallback` to let
        # eRPC's default selection policy treat this upstream as a
        # safety net used only when the primary tier is empty.
        tags:
          - tier:fallback
        ...
        evm:
          ...
          # (OPTIONAL) statePollerDebounce overrides the debounce interval for block polling.
          # When omitted, the interval is dynamically inferred from the chain's observed block
          # time. This skips redundant RPC calls when
          # request traffic already keeps the block number fresh via response enrichment.
          # Only set this if you need to override the automatic behavior.
          # DEFAULT: dynamic (inferred from observed block time)
  ...
  projects: [
    {
      ...
      upstreams: [
        // (REQUIRED) Endpoint URL supports http(s) scheme along with custom schemes like "alchemy://" defined below in this docs.
        {
          ...
          /*
          * (OPTIONAL) Tags are arbitrary `<dimension>:<value>` labels
          * consumed by the selectionPolicy stdlib (byTag / preferTag /
          * spreadAcrossTags). Use `tier:fallback` to mark this upstream
          * as a safety net used only when the primary tier is empty.
          */
          tags: ["tier:fallback"],
          ...
          evm: {
            ...
            */
            ...
            statePollerInterval: "30s",
            // (OPTIONAL) statePollerDebounce overrides the debounce interval for block polling.
            // When omitted, dynamically inferred from observed block time.
            // DEFAULT: dynamic (inferred from observed block time)
            // statePollerDebounce: "2s",
            // (OPTIONAL) nodeType is optional and you can manually set it to "full" or "archive".
            // DEFAULT: archive
...
projects:
  - id: main
    ...
    upstreamDefaults:
      # Default tags applied to every upstream that doesn't override them.
      tags:
        - tier:default
...
The upstream fields that the selection policy reads:

- `id` — referenced from `selectionPolicy.eval` and admin endpoints.
...
The legacy `routing.scoreMultipliers` block is gone. Per-upstream score weights are now expressed as a function passed to `sortByScore(...)`. See the [migration guide](/migration/selection-policy#worked-examples) for the legacy → new mapping.
...
<Callout type='info'>
  ...
  <br /><br />
  A parallel <code>traceFilterAutoSplittingRangeThreshold</code> upstream hint enables the same splitting behavior for <code>trace_filter</code> and <code>arbtrace_filter</code>. Network-level knobs (<code>traceFilterSplitOnError</code>, <code>traceFilterSplitConcurrency</code>) live in the same page — see <a href="/config/projects/networks#evm-networks">EVM Networks</a> → <em>trace_filter and arbtrace_filter</em>.
</Callout>
