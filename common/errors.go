...
// ErrEvmSafeBlockUnavailable is returned for a `safe`-tagged request on a
// network that configured an authoritative source for the tag
// (evm.safeBlock.source) when the tag could not be resolved from that source.
//
// This is deliberately fail-closed. The alternative — forwarding `safe`
// verbatim — would let whichever provider answers define the tag with its own
// (possibly zero) L1 confirmation depth, silently returning a weaker guarantee
// than the operator asked for. An explicit error is recoverable by the caller;
// a quietly-weaker answer is not.
//
// `reason` distinguishes the two ways this happens, because they need different
// operator responses: no source is currently reporting a safe head (an
// availability problem) versus the caller asked to skip interpolation (a client
// cannot opt out of an operator's trust boundary).
type ErrEvmSafeBlockUnavailable struct{ BaseError }

const ErrCodeEvmSafeBlockUnavailable ErrorCode = "ErrEvmSafeBlockUnavailable"

// Reasons carried by ErrEvmSafeBlockUnavailable.
const (
	// SafeBlockUnresolvedNoSource — no upstream matching evm.safeBlock.source
	// currently reports a safe head (cold start, all syncing, or the tag is
	// unsupported there).
	SafeBlockUnresolvedNoSource = "no_authoritative_source_reporting"
	// SafeBlockUnresolvedSkipInterpolation — the request carried the
	// skip-interpolation directive, so the tag was never resolved. Honoring the
	// directive here would mean forwarding a verbatim `safe` that any provider
	// could answer with its own confirmation depth, so the trust boundary wins
	// over the directive.
	SafeBlockUnresolvedSkipInterpolation = "interpolation_skipped_by_directive"
)

var NewErrEvmSafeBlockUnavailable = func(networkId string, source string, reason string) error {
	msg := "refusing to answer a 'safe'-tagged request from non-authoritative upstreams"
	switch reason {
	case SafeBlockUnresolvedNoSource:
		msg = "no authoritative 'safe' block is currently known for this network; refusing to resolve the safe tag from non-authoritative upstreams"
	case SafeBlockUnresolvedSkipInterpolation:
		msg = "cannot skip interpolation for a 'safe'-tagged request on a network with a configured authoritative safe source; forwarding the tag verbatim would let a non-authoritative upstream define it"
	}
	return &ErrEvmSafeBlockUnavailable{
		BaseError{
			Code:    ErrCodeEvmSafeBlockUnavailable,
			Message: msg,
			Details: map[string]interface{}{
				"networkId": networkId,
				"source":    source,
