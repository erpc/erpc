...
// MatchesRequest checks if this failsafe config matches the given request
func (f *FailsafeConfig) MatchesRequest(networkId, method string, params []interface{}, finality DataFinalityState) bool {
	// If new matchers are defined, use them
	if len(f.Matchers) > 0 {
		matcher := NewConfigMatcher(f.Matchers)
		result := matcher.MatchRequest(networkId, method, params, finality)
		return result.Matched && result.Action == MatcherInclude
	}

	// Fallback to legacy matching for backward compatibility
	return f.matchesLegacy(method, finality)
}

// matchesLegacy provides backward compatibility with the old MatchMethod/MatchFinality approach
func (f *FailsafeConfig) matchesLegacy(method string, finality DataFinalityState) bool {
	// Match method using wildcard
	if f.MatchMethod != "" && f.MatchMethod != "*" {
		matched, err := WildcardMatch(f.MatchMethod, method)
		if err != nil || !matched {
			return false
		}
	}
	...
	if len(f.MatchFinality) > 0 {
		found := false
		for _, expectedFinality := range f.MatchFinality {
...
func NewConfigMatcher(configs []*MatcherConfig) *ConfigMatcher {
	return &ConfigMatcher{
		configs: configs,
	}
}

// ConfigMatcher handles matching logic for requests using MatcherConfig
type ConfigMatcher struct {
	configs []*MatcherConfig
}

// MatchRequest evaluates if a request matches based on the configs
func (m *ConfigMatcher) MatchRequest(networkId, method string, params []interface{}, finality DataFinalityState) MatchResult {
	if len(m.configs) == 0 {
		return MatchResult{Matched: true, Action: MatcherInclude}
	}

	for _, config := range m.configs {
		if m.matchConfig(config, networkId, method, params, finality) {
			return MatchResult{Matched: true, Action: config.Action}
		}
	}

	// Default behavior if no matchers match
	return MatchResult{Matched: false, Action: MatcherExclude}
}

// matchConfig checks if all fields in the config match
func (m *ConfigMatcher) matchConfig(config *MatcherConfig, networkId, method string, params []interface{}, finality DataFinalityState) bool {
	// Match network
	if config.Network != "" {
...
func matchParam(pattern interface{}, param interface{}) (bool, error) {
	...
	case []interface{}:
		...
		return true, nil
	case string:
		match, err := WildcardMatch(p, paramToString(param))
		if err != nil {
			return false, err
		}
		return match, nil
