...
#### `enforceNonNullBlocks`

Controls whether null/empty responses from `eth_getBlockByNumber` should be converted to `ErrEndpointMissingData` errors.

- **Default**: `true` (existing behavior - null responses become errors)
- **When `false`**: Null responses are returned as-is without conversion to errors
- **Use case**: Useful for pending blocks that don't exist yet, or when you want to handle null responses at the application level

**Example:**
```yaml
evm:
  integrity:
    enforceNonNullBlocks: false  # Allow null block responses
```

When disabled, requests for non-existent blocks (like pending blocks that haven't been created yet) will return `{"result": null}` instead of an error.

**Interaction with `retry-empty` directive:**
- `enforceNonNullBlocks: false` → Returns null, no error, no retry
- `enforceNonNullBlocks: true` + `retry-empty: false` → Creates error, doesn't retry
- `enforceNonNullBlocks: true` + `retry-empty: true` → Creates error, retries other upstreams
      ...
      - architecture: evm
        evm:
          ...
          integrity:
            ...
            enforceNonNullBlocks: true         # Default: true - Convert null block responses to errors
