...
type config struct {
	...
	fireAndForget           bool
	maxWaitOnResult         time.Duration
	maxWaitOnEmpty          time.Duration
}
...
func (b *builder) WithMaxWaitOnResult(d time.Duration) *builder {
	b.cfg.maxWaitOnResult = d
	return b
}
func (b *builder) WithMaxWaitOnEmpty(d time.Duration) *builder {
