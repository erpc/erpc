...
func validateMatchers(matchers []*MatcherConfig) ([]*MatcherConfig, error) {
	...
	// Check if this is a single auto-generated default matcher from ConvertFailsafeLegacyMatchers
	// If so, skip the global override logic to avoid breaking user expectations
	if len(matchers) == 1 {
		matcher := matchers[0]
		isAutoGeneratedDefault := matcher.Method == "*" &&
			matcher.Network == "" &&
			len(matcher.Params) == 0 &&
			len(matcher.Finality) == 0 &&
			matcher.Action == MatcherInclude

		if isAutoGeneratedDefault {
			// This is the auto-generated default catch-all matcher
			// Don't apply global override logic - just return it as-is
			// This ensures failsafe policies work by default when no explicit matchers are provided
			return matchers, nil
		}
	}
