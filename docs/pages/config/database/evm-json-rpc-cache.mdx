...
You can enable compression for any connector to reduce storage size and potentially avoid size limitations. This is particularly useful for large JSON-RPC responses like blocks with many transactions or trace results.
  ...
  evmJsonRpcCache:
    connectors:
      - id: "postgres-cache"
        driver: "postgresql"
        postgresql:
          connectionUri: "postgres://user:password@localhost:5432/erpc"
          table: "cache"
        # Compression configuration
        compression:
          type: "zstd"  # Options: "none", "gzip", "zstd"
          level: 3      # Compression level (higher = better compression but slower)
```
</Tab>
<Tab>
```ts filename="erpc.ts"
...
- **Type**:
  ...
  - `zstd`: Zstandard compression (better performance and compression ratio)

- **Level**:
  - For `gzip`: 1-9 (1 = fastest, 9 = best compression)
  - For `zstd`: 1-22 (1 = fastest, 22 = best compression)
...
- Can reduce network transfer costs with remote databases
- Works with all connector types

**Trade-offs:**
- Adds CPU overhead for compression/decompression (typically single-digit ms)
- Slightly increases latency for cache operations
- Very small responses may not benefit from compression
...
For maximum compression (when storage is critical):
```yaml
compression:
  type: "zstd"
  level: 19  # Very high compression, but slower
```

For maximum speed (when CPU is critical):
```yaml
compression:
  type: "zstd"
