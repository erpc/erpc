...
- `unknown`: When block number cannot be determined from request/response (e.g., `eth_traceTransaction`), or when upstream does not support 'finalized' tag in eth_getBlockByNumber.
