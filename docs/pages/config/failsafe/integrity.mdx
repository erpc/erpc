...
* Hard limit on the max allowed range for eth_getLogs, set with [`upstream.evm.GetLogsMaxAllowedRange` config](/config/projects/upstreams#eth_getlogs-max-allowed-range) set per-upstream and rejects getLogs request if the range is beyond the limit and will not be split further.
