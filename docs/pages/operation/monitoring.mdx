...
| erpc_upstream_evm_get_logs_range_exceeded_total    | Counter   | Total number of times eth_getLogs request exceeded the maximum allowed block range and needed splitting (based on upstream config for `upstream.evm.getLogsMaxBlockRange`).                   |
