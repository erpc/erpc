        ...
        scoreMultipliers: [
          {
            ...
            method: "*", // Method(s) where you want to apply these multipliers (default: all methods)
            // method: 'eth_*|alchemy_*' means apply these multipliers to all methods starting with "eth_" or "alchemy_"
            finality: ["finalized", "unfinalized"], // Finality states for this multiplier set (default: all finality states)
            // Can be: "finalized", "unfinalized", "realtime", or "unknown"
...
### Finality-aware score multipliers

You can configure different scoring weights based on data finality. This is useful when different upstreams excel at different finality states:
...
upstreams:
  ...
  - id: my-fast-node
    ...
    routing:
      scoreMultipliers:
        # High latency weight for realtime requests (need low latency for fast data)
        - finality: [realtime]
          respLatency: 10.0
          blockHeadLag: 2.0
          overall: 1.0
        
        # High error rate weight for finalized requests (reliability matters most)
        - finality: [finalized]
          errorRate: 8.0
          respLatency: 2.0
          blockHeadLag: 1.0
        ...
        # Balanced scoring for unfinalized requests
...
<Callout type='info'>
  Leave `finality` empty (omit it) to apply the multiplier to all finality states. Multiple finality states can be specified in the array.
</Callout>
