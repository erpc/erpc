...
**Selection Policy** allows you to influence how upstreams are selected to serve (or not) traffic. A selection policy is defined at the network level and is responsible for returning a list of upstreams that must remain active. 
...
<Callout type='info'>
   By default, a "default" selection policy is activated if at least one upstream is assigned to the "fallback" group. This default policy incorporates basic logic for error rates and block lag, which can be tuned via theese environment variables 
...
- **Block Lag Management:** Disable upstreams that are lagging behind more than a specified number of blocks until they resync.
- **Error Rate Handling:** Exclude upstreams exceeding a certain error rate and periodically check their status.
- **Cost-Efficiency Optimization:** Prioritize "cheap" nodes and fallback to "fast" nodes only when necessary.
...
export type UpstreamMetrics = {
  ...
  // 90th percentile latency in seconds for this upstream.
