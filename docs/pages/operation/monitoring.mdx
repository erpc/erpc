...
| erpc_upstream_multiplexed_polls_total             | Counter   | Total number of multiplexed poll operations for upstream state polling.
...
# Multiplexed poll operations by finalized poll operation
sum(rate(erpc_upstream_multiplexed_polls_total{operation="finalized"}[5m])) by (network, upstream)
```