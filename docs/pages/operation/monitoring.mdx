...
| erpc_network_latest_block_timestamp_distance_seconds | Gauge   | Distance in seconds between the latest block timestamp and current time for a network. Includes `origin` label to distinguish between `evm_state_poller` (from state poller) and `network_response` (from actual responses). Useful for monitoring chain synchronization, block production latency, and cache staleness. |
