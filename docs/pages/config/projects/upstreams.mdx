...
upstreams:
  - id: my-infura
    jsonRpc:
      enableGzip: true  # Enable gzip when sending requests to this upstream
```
...
```yaml
server:
  enableGzip: true  # Disable gzip compression for responses to clients
```

<Callout type='info'>
  Response compression (eRPC → Client) is enabled by default when clients support it. Upstream compression need to be explicitly enabled if provider supports it.
</Callout>
