...
A simple strategy that allows you to define a secret value that will be checked against a provided `token` provided as a query string parameter, or as a `X-ERPC-Secret-Token` header.

<Callout type="info">
    This strategy mainly recommended for backend to backend communication. Exposing this token on your frontend allows users to impersonate the requests from anywhere.
...
```bash
curl -X POST https://localhost:4000?token=some-random-secret-value \
  # ...
```
...
<Callout type="info">
    For frontend dApps this strategy is the **most recommended** because it allows control over how many users can hit your RPC endpoint and the "expiration" prevents users from abusing the RPC by copying the RPC in multiple places.

    If you already use a JWT for your frontend, you can use the same token for eRPC, only providing the proper public key(s).
</Callout>

This strategy respects the JWT token's expiration (`exp` claim) and will reject the token if it has expired.
...
Many frontend dApps already use [Sign-in with Ethereum](https://eips.ethereum.org/EIPS/eip-4361) (SIWE) to authenticate wallets. You can use `siwe` strategy to allow requests from these dApps, by providing the signature and signed message:
