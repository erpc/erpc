...
<br />- [ ] Add a "exclusiveMethods" to make it easier to configure which upstreams
are only ones supporting certain methods (e.g. alchemy_*) - [ ] Automatically detect
type of EVM nodes (full, archive) and engines (erigon, geth, etc) - [ ] Implement
the logic for "getLogsMaxBlockRange" to split multiple requests when the block range
is too high. - [ ] Add more special types for well-known vendors (BlastAPI, Ankr,
LlamaRPC, etc) for easier multi-chain support.
