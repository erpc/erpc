...
func TestMatchFailsafeConfigs(t *testing.T) {
	t.Run("EmptyMatchersAlwaysMatch", func(t *testing.T) {
		// Both have no matchers - should match
		fs1 := &FailsafeConfig{}
		fs2 := &FailsafeConfig{}
		assert.True(t, matchFailsafeConfigs(fs1, fs2))

		// One has matchers, other doesn't - should match (backward compatibility)
		fs3 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{Method: "eth_call"}},
		}
		fs4 := &FailsafeConfig{}
		assert.True(t, matchFailsafeConfigs(fs3, fs4))
		assert.True(t, matchFailsafeConfigs(fs4, fs3))
	})

	t.Run("MethodMatching", func(t *testing.T) {
		// Exact match
		fs1 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{Method: "eth_call"}},
		}
		...
		// Wildcard match
		fs3 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{Method: "eth_*"}},
		}
		fs4 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{Method: "eth_call"}},
		}
		...
		fs6 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{Method: "eth_getBalance"}},
		}
		assert.False(t, matchFailsafeConfigs(fs5, fs6))

		// One empty method (matches any)
		fs7 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{Method: ""}},
		}
		fs8 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{Method: "eth_call"}},
		}
		assert.True(t, matchFailsafeConfigs(fs7, fs8))
		assert.True(t, matchFailsafeConfigs(fs8, fs7))
	})

	t.Run("NetworkMatching", func(t *testing.T) {
		// Exact match
		fs1 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{Network: "evm:1"}},
		}
		fs2 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{Network: "evm:1"}},
		}
		...
		// Wildcard match
		fs3 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{Network: "evm:*"}},
		}
		fs4 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{Network: "evm:137"}},
		}
		assert.True(t, matchFailsafeConfigs(fs4, fs3))
	...
	t.Run("FinalityMatching", func(t *testing.T) {
		// Overlap match
		fs1 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{
				Finality: []DataFinalityState{DataFinalityStateFinalized, DataFinalityStateRealtime},
			}},
		}
		fs2 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{
		...
		fs6 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{
				Finality: []DataFinalityState{DataFinalityStateRealtime},
			}},
		}
		assert.False(t, matchFailsafeConfigs(fs5, fs6))
	})

	t.Run("ParamsMatching", func(t *testing.T) {
		// Exact match
		fs1 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{
				Params: []interface{}{"0x123", 100},
			}},
		}
		fs2 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{
		...
		fs4 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{
				Params: []interface{}{"0x456", 100},
			}},
		}
		assert.False(t, matchFailsafeConfigs(fs3, fs4))

		// One empty params (matches any)
		fs5 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{
				Params: []interface{}{},
			}},
		...
		assert.True(t, matchFailsafeConfigs(fs5, fs6))
		assert.True(t, matchFailsafeConfigs(fs6, fs5))
	})

	t.Run("CombinedMatching", func(t *testing.T) {
		// All fields match
		fs1 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{
				Network:  "evm:1",
		...
		fs4 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{
				Network:  "evm:137", // Different network
				Method:   "eth_call",
				Finality: []DataFinalityState{DataFinalityStateFinalized},
			}},
		}
		...
		// Wildcard in defaults matches specific upstream
		fs5 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{
				Network: "evm:*",
				Method:  "eth_*",
	...
	t.Run("LegacyFieldConversion", func(t *testing.T) {
		// Legacy fields should be converted to matchers
		fs1 := &FailsafeConfig{
	...
	t.Run("RealWorldScenarios", func(t *testing.T) {
		// Scenario 1: Default with wildcard, upstream with specific
		...
		upstreamFs := &FailsafeConfig{
			Matchers: []*MatcherConfig{{
				Method: "eth_call",
			}},
			Retry: &RetryPolicyConfig{MaxAttempts: 3},
		}
		assert.True(t, matchFailsafeConfigs(upstreamFs, defaultFs))

		// Scenario 2: Network-specific defaults
		defaultFs2 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{
				Network: "evm:1",
				Method:  "*",
			}},
			Timeout: &TimeoutPolicyConfig{Duration: Duration(60 * time.Second)},
		}
		upstreamFs2 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{
				Network: "evm:1",
				Method:  "eth_getLogs",
			}},
			Hedge: &HedgePolicyConfig{Delay: Duration(100 * time.Millisecond)},
		}
		assert.True(t, matchFailsafeConfigs(upstreamFs2, defaultFs2))

		// Scenario 3: No match due to different networks
		defaultFs3 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{
				Network: "evm:1",
			}},
		}
		upstreamFs3 := &FailsafeConfig{
			Matchers: []*MatcherConfig{{
				Network: "evm:137",
			}},
		}
		assert.False(t, matchFailsafeConfigs(upstreamFs3, defaultFs3))
	})
}
