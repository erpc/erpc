...
| `hedge.delay.quantile` | `float64` | `0` (static mode) | Percentile queried from the per-method `QuantileTracker`. Valid range `[0, 1]`. When `> 0`, `base` or `max` must also be set (enforced by `AdaptiveDuration.validate` at <SourceLink file="common/adaptive_duration.go" lines="129" />). At cache-connector scope (`failsafeForGets` / `failsafeForSets`) the quantile resolves against the connector's own per-(method, finality) latency window instead of network metrics — <SourceLink file="data/cache_executor.go" lines="204-207" />. System template default: `0.7`. Source: <SourceLink file="common/adaptive_duration.go" lines="83-109" /> |
...
**6. Quantile hedge on cache-connector reads.** Cache connectors take their own failsafe via
`failsafeForGets` / `failsafeForSets`. A quantile delay resolves against the connector's own
latency — a 1-minute sliding window tracked per (method, finality) executor, fed only by
completed operations (<SourceLink file="data/cache_executor.go" lines="14-20" />) — so the
hedge tracks the shared cache's real speed (e.g. a remote gRPC cache connector). Before any
samples exist the delay falls back to `base + min` (<SourceLink file="common/adaptive_duration.go" lines="96-101" />):
...
<ConfigTabs
  ...
  - id: remote-cache
    ...
    failsafeForGets:
      - matchMethod: "*"
        ...
        hedge:
          delay:
            quantile: 0.95  # p95 of this connector's own completed reads (1m window)
            min: 20ms       # floor; also cold-start fallback (no samples → base+min)
            max: 200ms      # a hedge later than this is pointless — timeout is 400ms
          maxCount: 1
        retry:
          maxAttempts: 2
          delay: "0"`}
  ...
  failsafeForGets: [{
    ...
    // p95 of this connector's own completed reads (1m window);
    // cold start (no samples) falls back to base+min
    hedge: { delay: { quantile: 0.95, min: "20ms", max: "200ms" }, maxCount: 1 },
