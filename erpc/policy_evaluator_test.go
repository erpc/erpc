...
func TestPolicyEvaluator(t *testing.T) {
	...
	t.Run("BasicEvaluation", func(t *testing.T) {
		...
		mt.RecordUpstreamDuration(ups2, "method1", 10*time.Millisecond, true, "none", common.DataFinalityStateUnknown, "n/a")
	...
	t.Run("UpstreamRecovery", func(t *testing.T) {
		...
		mt.RecordUpstreamRequest(ups1, "method1")
		mt.RecordUpstreamDuration(ups1, "method1", 15*time.Millisecond, true, "none", common.DataFinalityStateUnknown, "n/a")
		mt.RecordUpstreamRequest(ups1, "method1")
		mt.RecordUpstreamDuration(ups1, "method1", 12*time.Millisecond, true, "none", common.DataFinalityStateUnknown, "n/a")
	...
	t.Run("MetricsUpdate", func(t *testing.T) {
		...
		mt.RecordUpstreamDuration(ups1, "method1", 15*time.Millisecond, true, "none", common.DataFinalityStateUnknown, "n/a")
		mt.RecordUpstreamRequest(ups1, "method1")
		mt.RecordUpstreamDuration(ups1, "method1", 20*time.Millisecond, true, "none", common.DataFinalityStateUnknown, "n/a")
		mt.RecordUpstreamRequest(ups1, "method1")
		mt.RecordUpstreamDuration(ups1, "method1", 25*time.Millisecond, true, "none", common.DataFinalityStateUnknown, "n/a")
			...
			for {
				...
				case <-ticker.C:
					...
					if rand.Float64() < errorRate {
						...
						mt.RecordUpstreamDuration(ups1, "method1", time.Duration(rand.Intn(100))*time.Millisecond, true, "none", common.DataFinalityStateUnknown, "n/a")
					}
				}
			}
		}()
		...
		go func() {
			...
			for {
				...
				case <-ticker.C:
					...
					mt.RecordUpstreamDuration(ups1, methodName, 50*time.Millisecond, true, "none", common.DataFinalityStateUnknown, "n/a")
					methodCounter++
				}
			}
		}()
	...
	t.Run("MethodSpecificMetricsIsolation", func(t *testing.T) {
		...
		mt.RecordUpstreamDuration(ups1, "method2", 10*time.Millisecond, true, "none", common.DataFinalityStateUnknown, "n/a")
		mt.RecordUpstreamRequest(ups1, "method2")
		mt.RecordUpstreamDuration(ups1, "method2", 15*time.Millisecond, true, "none", common.DataFinalityStateUnknown, "n/a")
	...
	t.Run("MethodSpecificGlobalFallback", func(t *testing.T) {
		...
		mt.RecordUpstreamDuration(ups1, "*", 10*time.Millisecond, true, "none", common.DataFinalityStateUnknown, "n/a")
	...
	t.Run("DefaultPolicyDisableFallbacksWhenDefaultsHealthy", func(t *testing.T) {
		...
		mt.SetLatestBlockNumber(ups3, 103, 0, "evm_state_poller")
	...
	t.Run("DefaultPolicyMinHealthyThreshold", func(t *testing.T) {
		...
		mt.RecordUpstreamRequest(ups2, "method1")
		mt.RecordUpstreamFailure(ups2, "method1", fmt.Errorf("test problem"))
		mt.SetLatestBlockNumber(ups2, 95, 0, "evm_state_poller") // Lagging
		...
		mt.SetLatestBlockNumber(ups3, 101, 0, "evm_state_poller")
		...
		mt.SetLatestBlockNumber(ups2, 101, 0, "evm_state_poller")
	...
	t.Run("DefaultPolicyBlockLagThreshold", func(t *testing.T) {
		...
		mt.SetLatestBlockNumber(ups1, 100, 0, "evm_state_poller") // Leader
		mt.SetLatestBlockNumber(ups2, 95, 0, "evm_state_poller")  // Lag of 5 blocks (exceeds threshold)
		mt.SetLatestBlockNumber(ups3, 99, 0, "evm_state_poller")  // Small lag for fallback (does not exceed threshold)
		...
		mt.SetLatestBlockNumber(ups3, 200, 0, "evm_state_poller") // Fallback becomes leader
		mt.SetLatestBlockNumber(ups1, 195, 0, "evm_state_poller") // Now lagging by 5 blocks (exceeds threshold)
		mt.SetLatestBlockNumber(ups2, 190, 0, "evm_state_poller") // Now lagging by 10 blocks (exceeds threshold)
	...
	t.Run("DefaultPolicyErrorRateThreshold", func(t *testing.T) {
		...
		mt.SetLatestBlockNumber(ups1, 99, 0, "evm_state_poller")
		mt.SetLatestBlockNumber(ups2, 99, 0, "evm_state_poller")
		mt.SetLatestBlockNumber(ups3, 99, 0, "evm_state_poller")
		...
		for i := 0; i < 10; i++ {
			mt.RecordUpstreamRequest(ups3, "method1")
			mt.RecordUpstreamDuration(ups3, "method1", 20*time.Millisecond, true, "none", common.DataFinalityStateUnknown, "n/a")
		}
...
func TestPolicyEvaluatorBlockHeadLagFlow(t *testing.T) {
	...
	t.Run("BlockHeadLagCordoningAndUncordoning", func(t *testing.T) {
		...
		mt.SetLatestBlockNumber(ups1, 100, 0, "evm_state_poller") // ups1 is the leader
		mt.SetLatestBlockNumber(ups2, 95, 0, "evm_state_poller")  // ups2 has lag of 5 blocks (< 10, so healthy)
		...
		mt.SetLatestBlockNumber(ups1, 120, 0, "evm_state_poller") // ups1 advances to 120 (still leader)
		mt.SetLatestBlockNumber(ups2, 105, 0, "evm_state_poller") // ups2 now has lag of 15 blocks (> 10, so unhealthy)
		...
		mt.SetLatestBlockNumber(ups2, 125, 0, "evm_state_poller") // ups2 catches up and becomes leader
		mt.SetLatestBlockNumber(ups1, 120, 0, "evm_state_poller") // ups1 now has lag of 5 blocks (< 10, so healthy)
		...
		mt.SetLatestBlockNumber(ups1, 130, 0, "evm_state_poller") // ups1 becomes leader
		mt.SetLatestBlockNumber(ups2, 130, 0, "evm_state_poller") // ups2 has 0 lag
	...
	t.Run("BlockHeadLagWithGroupFiltering", func(t *testing.T) {
		...
		mt.SetLatestBlockNumber(ups1, 100, 0, "evm_state_poller")
		mt.SetLatestBlockNumber(ups2, 95, 0, "evm_state_poller")
		...
		mt.SetLatestBlockNumber(ups1, 120, 0, "evm_state_poller")
		mt.SetLatestBlockNumber(ups2, 105, 0, "evm_state_poller") // lag of 15 blocks
		...
		// ups2 catches up
		mt.SetLatestBlockNumber(ups2, 118, 0, "evm_state_poller") // lag of 2 blocks
	...
	t.Run("BlockHeadLagWithEnvironmentVariable", func(t *testing.T) {
		...
		mt.SetLatestBlockNumber(ups1, 100, 0, "evm_state_poller")
		mt.SetLatestBlockNumber(ups2, 95, 0, "evm_state_poller") // lag of 5 blocks (exactly at threshold)
		...
		mt.SetLatestBlockNumber(ups2, 97, 0, "evm_state_poller") // lag of 3 blocks (< 5)
	...
	t.Run("BlockHeadLagRapidChanges", func(t *testing.T) {
		...
		for i := 0; i < 5; i++ {
			// ups1 advances
			mt.SetLatestBlockNumber(ups1, int64(100+i*10), 0, "evm_state_poller")
			mt.SetLatestBlockNumber(ups2, int64(100+i*10-3), 0, "evm_state_poller") // lag of 3 blocks (> 2, so unhealthy)
			...
			// ups2 catches up
			mt.SetLatestBlockNumber(ups2, int64(100+i*10-1), 0, "evm_state_poller") // lag of 1 block (< 2, so healthy)
