...
For large deployments with many networks and upstreams, these settings help control metric cardinality:

#### Histogram Label Mode

The biggest cardinality driver. Duration histograms (`upstream_request_duration_seconds`, `network_request_duration_seconds`) include per-upstream, per-finality, per-user, and per-composite labels by default. In compact mode, these collapse to `"n/a"` — keeping only network and method as dynamic labels.

- `detailed` (default): All labels on histograms. Highest cardinality.
- `compact`: Collapses upstream, finality, user, composite to `"n/a"`. **~80% reduction** in histogram series.

<Tabs items={["yaml", "typescript"]} defaultIndex={0} storageKey="GlobalConfigTypeTabIndex">
  <Tabs.Tab>
```yaml filename="erpc.yaml"
metrics:
  histogramLabelMode: "compact"
```
  </Tabs.Tab>
  <Tabs.Tab>
```ts filename="erpc.ts"
...
Per-upstream latency detail is lost, but per-upstream counters and health scores are unaffected.

#### Error Label Mode

Controls how detailed error information is included in error counter labels:

- `verbose` (default): Full error fingerprint as labels.
- `compact`: Groups similar errors by type only.
...
Fewer buckets = proportionally fewer series per histogram. The default (`0.05, 0.5, 5, 30`) is already minimal at 4 buckets. Only change if you need finer latency percentile resolution.
