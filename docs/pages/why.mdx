---
title: Why eRPC?
description: Main use-cases — cost reduction, fault tolerance, observability, and EVM-aware load balancing.
---
...
- [Selection policy](/config/projects/selection-policies) that scores upstreams in real time on response time, error rate, throttling, block lag, and misbehavior — and trips degraded ones out of rotation automatically. Operators can also [manually cordon](/operation/cordoning) an upstream during incidents.
- [Failsafe](/config/failsafe) policies to help with intermittent issues and increase general resiliency (retries, hedging, consensus).
