  ...
  - id: main
    ...
    networks:
      - architecture: evm
        ...
        # engine uses when `eval` is omitted — sortByScore(BALANCED)
        # plus preferGroup/stickyPrimary/probeExcluded. Override
        # `eval` to express any custom logic via the chainable stdlib.
        selectionPolicy:
          evalInterval: 1s
          evalTimeout: 100ms
          evalPerMethod: false
      ...
      networks: [
        {
           ...
           * (OPTIONAL) See /config/projects/selection-policies for the
           * full reference. The block below shows the defaults the
           * engine uses when `eval` is omitted.
           */
          selectionPolicy: {
            evalInterval: "1s",
            evalTimeout: "100ms",
