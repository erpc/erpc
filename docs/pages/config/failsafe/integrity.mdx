...
* Hard limit on the max allowed range for eth_getLogs, set with [`upstream.evm.GetLogsMaxAllowedRange` config](/config/projects/upstreams#eth_getlogs-max-allowed-range) set per-upstream and rejects getLogs request if the range is beyond the limit and returns an error to the caller of eRPC.
...
- `erpc_upstream_evm_get_logs_range_exceeded_hard_limit_total` - Total number of times eth_getLogs request exceeded the block range hard limit will not be split further (based on upstream config for "upstream.evm.GetLogsMaxAllowedRange").
