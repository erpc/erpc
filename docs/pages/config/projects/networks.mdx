  ...
  - id: main
    ...
    networks:
      - architecture: evm
        ...
        alias: ethereum
        # (OPTIONAL) See /config/projects/selection-policies for the
        # full reference. The block below shows the defaults the
        # engine uses when `evalFunc` is omitted — sortByScore(BALANCED)
        # plus preferTag/stickyPrimary/probeExcluded. Override
        # `evalFunc` to express any custom logic via the chainable stdlib.
        # Investigations: per-tick reasoning is exposed via tracing +
        # the `policy_selection_*` Prometheus metric family; there is
        # no in-memory decision-history buffer.
        selectionPolicy:
          evalInterval: 1s
          evalTimeout: 100ms
          evalPerMethod: false
      ...
      networks: [
        {
           ...
           * (OPTIONAL) See /config/projects/selection-policies for the
           * full reference. The block below shows the defaults the
           * engine uses when `evalFunc` is omitted.
           */
          selectionPolicy: {
            evalInterval: "1s",
            evalTimeout: "100ms",
