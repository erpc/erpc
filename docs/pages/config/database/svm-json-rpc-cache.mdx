...
**Finality reflects the commitment eRPC pinned.** Step 3 calls `resolveCommitment` — the same predicate the [commitment-injection hook](/reference/svm/commitment) uses — so classification tracks the level eRPC actually pinned on the request, not the level a node happens to default to. When nothing is pinned (no caller `commitment`, no `svm.commitment`, or injection legitimately skips the legacy encoding-string form), the response is classified `unfinalized`. That is deliberately stricter than the routing predicate `IsFinalizedCommitment`, which follows the node's documented `finalized` default: cache permanence is a one-way door — `finality: finalized` is the zero value an unset-TTL policy matches, so a wrong promotion pins a value forever while a wrong demotion costs one re-fetch. Set `svm.commitment` to have unpinned slot-addressed reads cached as final.

**Cache-key derivation.** Two keys, as in EVM, but both dimensions differ:

- **Partition key: `<networkId>:<slotRef>`, or `<networkId>:<cacheKeySuffix>:<slotRef>` when `networks[].cacheKeySuffix` is set.** `slotRef` is the request's `minContextSlot` when one is present in any object param, otherwise the literal `"*"`. This replaces EVM's `blockRef` dimension. Because `slotRef` is derived purely from the request params, a given `(method, params)` tuple always produces the same partition key on both `Set` and `Get` — so SVM always reads `ConnectorMainIndex` and never needs EVM's reverse-index wildcard fallback.
...
- **Cache key structure.** Partition key `<networkId>:<slotRef>`, or `<networkId>:<cacheKeySuffix>:<slotRef>` when `networks[].cacheKeySuffix` is set. `slotRef` is the request's `minContextSlot` or `"*"`. Request key `<method>:<sha256(method ‖ 0x00 ‖ json(params))>`, case-preserving. Always read from `ConnectorMainIndex` — no reverse index.
...
13. **Same-chain networks share partition keys unless `cacheKeySuffix` is set.** Same rule as EVM: empty suffix keeps `<networkId>:<slotRef>`; `cacheKeySuffix: systx` yields `<networkId>:systx:<slotRef>`. Source: [`architecture/svm/json_rpc_cache.go:L443-L454`](https://github.com/erpc/erpc/blob/main/architecture/svm/json_rpc_cache.go#L443-L454).
