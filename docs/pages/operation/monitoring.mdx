...
| erpc_selection_readmit_total                       | Counter   | Times an upstream transitioned excluded → in-rotation (driven by `readmitExcluded` or by no longer tripping `excludeIf`). Labels: project, network, method, upstream.                            |
