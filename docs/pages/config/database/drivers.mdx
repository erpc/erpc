  ...
  evmJsonRpcCache:
    connectors:
      - id: redis-cache
        driver: redis
        redis:
          # Preferred method: URI connection string
          ...
          # username: "myuser" # Optional, for non-default users
          # password: "mypassword"
          # db: 0

          # max wait to establish a connection
          initTimeout: 5s
          # max wait for reads
          getTimeout: 1s
          # max wait for writes
...
- When using URI, these parameters are included in the URI string:
  - `redis://[username]:[password]@[host][:port][/database]`
  - Database is optional and defaults to 0

- These parameters must be configured separately, even when using URI:
  - Connection pool size
  - TLS configuration
  - Timeout settings (initTimeout, getTimeout, setTimeout)

- If both URI and individual parameters are provided, the URI takes precedence for connection details, but explicit timeouts and connection pool size from the config will still be used.
